...
func TestOspfv2P2pLsa(t *testing.T) {
	...
	api := otg.NewOtgApi(t)
	// api.Api().SetVersionCompatibilityCheck(false)
...
func ospfv2P2pLsasOk(api *otg.OtgApi, tc map[string]interface{}) bool {
	...
	for _, m := range api.GetOspfv2Lsas() {
		...
		if len(nwSummaryLsas) == 1 &&
			...
			api.Testing().Logf("SummaryLsas got: %v", nwSummaryLsaId)
		}
		...
		if len(routerLsas) == 1 &&
			...
			if links[0].Type() == gosnappi.Ospfv2LinkType.POINT_TO_POINT &&
				...
				api.Testing().Logf("RouterLsas got: %v", advRouterId)
			}
			if links[1].Type() == gosnappi.Ospfv2LinkType.POINT_TO_POINT &&
				links[1].Metric() == 1 &&
				links[1].Id() == routerLsaLinkId &&
				links[1].Data() == routerLsaLinkData &&
				links[0].Type() == gosnappi.Ospfv2LinkType.STUB &&
				links[0].Metric() == 1 {
				lsaCount += 1
